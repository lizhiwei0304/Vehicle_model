...
VehiclePlugin::VehiclePlugin()
    : dataPtr(new VehiclePluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "VehiclePlugin");
  this->robot_namespace_ = "";
  this->dataPtr->directionState = VehiclePluginPrivate::FORWARD;
  //修改车轮的半径
  this->dataPtr->flWheelRadius = 0.8693;
  this->dataPtr->frWheelRadius = 0.8693;
  this->dataPtr->blWheelRadius = 0.8693;
  this->dataPtr->brWheelRadius = 0.8693;
}


//****此函数实现了转角、制动、节气门的控制以及车辆行驶方向的确定,需要标定handWheelLow和handWheelHigh//
void VehiclePlugin::OnVehicleCommand(const control_msgs::Control::ConstPtr &msg)
{
  ...
  switch (msg->shift_gears)
  {
    case control_msgs::Control::NEUTRAL:
      this->dataPtr->directionState = VehiclePluginPrivate::NEUTRAL;
      break;
    case control_msgs::Control::FORWARD:
      this->dataPtr->directionState = VehiclePluginPrivate::FORWARD;
      break;
    case control_msgs::Control::REVERSE:
      this->dataPtr->directionState = VehiclePluginPrivate::REVERSE;
      break;
    default:
      break;
  }
...
///实现了所有关节、车轮以及键盘的控制的加载///已修改///
void VehiclePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  gzwarn << "VehiclePlugin loading params" << std::endl;
  // shortcut to this->dataPtr
  VehiclePluginPrivate *dPtr = this->dataPtr.get();
  ...
  this->dataPtr->controlSub = nh.subscribe("Vehicle_control", 10, &VehiclePlugin::OnVehicleCommand, this);

  this->dataPtr->node.Subscribe("/Vehicle/reset",
      &VehiclePlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/Vehicle/stop",
      &VehiclePlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/cmd_vel", &VehiclePlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_gear",
      &VehiclePlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/cmd_mode",
      &VehiclePlugin::OnCmdMode, this);

  this->dataPtr->posePub = this->dataPtr->node.Advertise<ignition::msgs::Pose>(
      "/Vehicle/pose");
  this->dataPtr->consolePub =
    this->dataPtr->node.Advertise<ignition::msgs::Double_V>("/Vehicle/console");
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&VehiclePlugin::Update, this));

  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
        &VehiclePlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &VehiclePlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
void VehiclePlugin::OnCmdVel(const ignition::msgs::Pose &_msg)
{
...
/////////////////////////////////////////////////
void VehiclePlugin::OnCmdGear(const ignition::msgs::Int32 &_msg)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  this->dataPtr->directionState =
      static_cast<VehiclePluginPrivate::DirectionType>(state);
}

///修改车辆的模式，发动机还是电动///修改，用不着，但是可以保留
void VehiclePlugin::OnCmdMode(const ignition::msgs::Boolean &/*_msg*/)
{
  // toggle ev mode
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
///键盘控制的加载控制车辆的节气门///不改
void VehiclePlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
///键盘控制的加载控制车辆的状态变量///不改
void VehiclePlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != VehiclePluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
///选择控制的方式///
void VehiclePlugin::KeyControl(const int _key)
{
...
///不要修改///
void VehiclePlugin::OnKeyPress(ConstAnyPtr &_msg)
{
  this->KeyControl(_msg->int_value());
}

///不要修改///
void VehiclePlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{
  this->KeyControl(_msg.int_value());
}

///不要修改///
void VehiclePlugin::OnReset(const ignition::msgs::Any & /*_msg*/)
{
...
///不要修改///
void VehiclePlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
...
void VehiclePlugin::Update()
{
  ...
  bool neutral = dPtr->directionState == VehiclePluginPrivate::NEUTRAL;
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    if (this->dataPtr->directionState == VehiclePluginPrivate::FORWARD)
      consoleMsg.add_data(1.0);
    else if (this->dataPtr->directionState == VehiclePluginPrivate::REVERSE)
      consoleMsg.add_data(2.0);
    else if (this->dataPtr->directionState == VehiclePluginPrivate::NEUTRAL)
...
///车辆自身参数的设置，方向盘转角-900-900，车轮转角-50-50////
void VehiclePlugin::UpdateHandWheelRatio()
{
...
double VehiclePlugin::CollisionRadius(physics::CollisionPtr _coll)
{
...
/////////////////////////////////////////////////
double VehiclePlugin::GasTorqueMultiplier()
{
